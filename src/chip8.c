...
int chip8_execute(chip8_t* chip8) {
	...
	switch (nibble1) {
		case 0:
			...
			break;
		case 1:	// 1nnn - JMP 
			if (opcode_jmp(chip8, ((nibble2 << 8) | (nibble3 << 4) | (nibble4))) != 0)
				return -1;
			break;
		case 2: // 2nnn - CALL
			if (opcode_call(chip8, ((nibble2 << 8) | (nibble3 << 4) | (nibble4))) != 0)
				return -1;	
			break;
...
chip8_t* chip8_init(const char* romPath) {
...
int main(int argc, char** argv) {
	...
	chip8_t* chip8 = chip8_init(argv[1]);
	if (chip8 == NULL) {
		SDL_Quit();
		return 1;
	}
